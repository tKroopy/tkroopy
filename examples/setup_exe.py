...
USAGE = """
Usage:
    Copy this and setup_exe.conf to tKroopy root folder
...
if len(sys.argv) != 2 or not os.path.isfile('tKroopy.py'):
    print USAGE
    sys.exit(1)
...
#tkroopy_version_line = readlines_file('VERSION')[0]
#use regular expression to get just the version number
#v_re = re.compile('[0-9]+\.[0-9]+\.[0-9]+')
#tkroopy_version = v_re.search(web2py_version_line).group(0)
...
if BUILD_MODE == 'py2exe':
    ...
    setup(
        #console=[{'script':'tKroopy.py',
        #          'icon_resources': [(0, 'images/tKroopy.ico')]
        #          }],
        windows=[{'script':'tKroopy.py',
                  'icon_resources': [(1, 'images/tKroopy.ico')],
                  'dest_base':'tKroopy'  # MUST NOT be just 'web2py' otherwise it overrides the standard web2py.exe
                  }],
        name="tKroopy",
...
if python_version > (2,5):
    ...
    try:
        shutil.copytree('C:\Program Files (x86)\ArcGIS\Desktop10.2\bin\Microsoft.VC90.CRT', 'dist/Microsoft.VC90.CRT/')
    except:
...
if make_zip:
    ...
    shutil.copytree('dist', 'zip_temp/tkroopy')
    #create zip file
...
print "Finished!"
#print "Enjoy web2py " + tkroopy_version_line